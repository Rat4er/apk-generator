...
from apk_worker.apk_worker import generate_apk, return_io_stream, generate_apk_v2, use_default_keystore, cleanup
...
async def generate_apk_v2_route():
    package_name, version_code, version_name, size_apk, keystore_path, keystore_pass, keystore_alias, keystore_keypass = await parse_form(request, app.config['UPLOAD_FOLDER'])
    try:
        final_apk = await generate_apk_v2(package_name, version_code, version_name, size_apk, keystore_path, keystore_pass, keystore_alias, keystore_keypass)
        response = await return_io_stream(final_apk)
        return response
    except Exception as e:
        return jsonify({'error': str(e)}), 500
    finally:
